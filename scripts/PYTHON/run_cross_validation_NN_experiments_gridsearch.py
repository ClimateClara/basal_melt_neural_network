...
layer_nb = int(sys.argv[1]) #'mini', 'small', 'medium', 'large', 'extra_large'
layer_size = int(sys.argv[2]) #'mini', 'small', 'medium', 'large', 'extra_large'
tblock_out = int(sys.argv[3])
isf_out = int(sys.argv[4])
TS_opt = str(sys.argv[5]) # extrap, whole, thermocline
norm_method = str(sys.argv[6]) # std, interquart, minmax
exp_name = str(sys.argv[7])
...
outputpath_nn_models = '/bettik/burgardc/DATA/NN_PARAM/interim/NN_MODELS/grid_search/'
...
model = modf.get_custom_model(layer_nb,layer_size,input_size,activ_fct,1)
...
model.save(path_model + 'model_nn_'+str(layer_nb)+'_'+str(layer_size)+'_'+exp_name+'_noisf'+str(isf_out).zfill(3)+'_notblock'+str(tblock_out).zfill(3)+'_TS'+TS_opt+'_norm'+norm_method+'.h5')

# convert the history.history dict to a pandas DataFrame:     
hist_df = pd.DataFrame(history.history) 

hist_csv_file = path_model + 'history_'+str(layer_nb)+'_'+str(layer_size)+'_'+exp_name+'_noisf'+str(isf_out).zfill(3)+'_notblock'+str(tblock_out).zfill(3)+'_TS'+TS_opt+'_norm'+norm_method+'.csv'
